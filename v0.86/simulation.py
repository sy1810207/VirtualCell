...
class VirtualCellSimulation:
    ...
    def run_phase2(self, groove_depth=None):
        ...
        # Position cell above the groove floor at x=-5, y=0
        sub_z = self.sub_vertices[:, 2]
        groove_floor_z = sub_z.min()
        groove_center_x = -5.0
        log.info(f'  Groove floor z={groove_floor_z:.2f}, target x={groove_center_x:.2f}')
        ...
        offset = np.array([groove_center_x - cell_center[0],
                           -cell_center[1],
        ...
        correction = np.array([-5.0 - actual_center[0],
                               0.0 - actual_center[1],
