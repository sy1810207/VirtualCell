...
def active_forces(cell_vertices, cell_faces, sub_vertices, sub_faces,
                  config: SimConfig):
    """
    Push membrane nodes near substrate outward along curvature vector.
    Net force balanced to zero.
...
def steric_substrate_repulsion(cell_vertices, sub_vertices, sub_faces,
                               config: SimConfig):
    """
    ...
    # Penetration: cell vertex below substrate
    penetration = sub_z + config.d_steric - cell_vertices[:, 2]
    mask = penetration > 0
    if np.any(mask):
        forces[mask, 2] += config.k_steric * penetration[mask]
