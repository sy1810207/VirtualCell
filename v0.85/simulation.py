...
class VirtualCellSimulation:
    ...
    def _compute_forces_phase2(self):
        ...
        # Cell-ECM LJ adhesion
        f_ecm, e_ecm = cell_ecm_forces(
            self.cell_vertices, self.cell_faces,
            self.sub_vertices, self.sub_faces, cfg)
        f_cell += f_ecm
        total_energy += e_ecm

        # Active force (spreading)
        f_act = active_forces(
            self.cell_vertices, self.cell_faces,
            self.sub_vertices, self.sub_faces, cfg)
        f_cell += f_act

        # Steric: cell membrane above substrate
        f_sub_steric = steric_substrate_repulsion(
            self.cell_vertices, self.sub_vertices, self.sub_faces, cfg)
        f_cell += f_sub_steric

        # Steric: nucleus above substrate
        f_nuc_sub = steric_substrate_repulsion(
            self.nuc_vertices, self.sub_vertices, self.sub_faces, cfg)
        f_nuc += f_nuc_sub
    ...
    def _update_positions(self, f_cell, f_nuc, f_cyt, f_chains):
        ...
        if self.sub_vertices is not None:
            ...
            sub_centroids = (self.sub_vertices[self.sub_faces[:, 0]] +
                             self.sub_vertices[self.sub_faces[:, 1]] +
                             self.sub_vertices[self.sub_faces[:, 2]]) / 3.0
            tree_2d = cKDTree(sub_centroids[:, :2])
            floor_margin = self.cfg.sigma_perp * 0.5

            # Nucleus vertices
            _, nidx = tree_2d.query(self.nuc_vertices[:, :2], k=1)
            nuc_floor = sub_centroids[nidx, 2] + floor_margin
            below = self.nuc_vertices[:, 2] < nuc_floor
            if np.any(below):
                self.nuc_vertices[below, 2] = nuc_floor[below]

            # Cytoplasm nodes
            _, cidx = tree_2d.query(self.cyt_positions[:, :2], k=1)
            cyt_floor = sub_centroids[cidx, 2] + floor_margin
            ...
            # Chromatin beads
            for i, chain in enumerate(self.chains):
                _, kidx = tree_2d.query(chain[:, :2], k=1)
                ch_floor = sub_centroids[kidx, 2] + floor_margin
    ...
    def run_phase2(self, groove_depth=None):
        ...
        # Position cell above substrate in the attractive LJ range
        # LJ 10-4 equilibrium: d_eq ≈ 1.165*sigma; place slightly above
        d_eq = 2.0 * cfg.sigma_perp  # above equilibrium → ECM pulls down
        sub_z_max = self.sub_vertices[:, 2].max()
        cell_center = self.cell_vertices.mean(axis=0)
        cell_bottom_z = self.cell_vertices[:, 2].min()
        cell_R_actual = cell_center[2] - cell_bottom_z
        target_z = sub_z_max + cell_R_actual + d_eq
        offset = np.array([0.0, 0.0, target_z - cell_center[2]])
        self.cell_vertices += offset
        self.nuc_vertices += offset
        self.cyt_positions += offset
        for i in range(len(self.chains)):
            self.chains[i] += offset
        log.info(f'  Cell placed at z={target_z:.2f} (bottom at z='
                 f'{(self.cell_vertices[:,2].min()):.2f}) above substrate')
    ...
    def _check_penetration(self, step):
        ...
        if self.sub_vertices is not None:
            ...
            sub_centroids = (self.sub_vertices[self.sub_faces[:, 0]] +
                             self.sub_vertices[self.sub_faces[:, 1]] +
                             self.sub_vertices[self.sub_faces[:, 2]]) / 3.0
            tree = cKDTree(sub_centroids[:, :2])
            _, nearest = tree.query(self.cell_vertices[:, :2], k=1)
            sub_z = sub_centroids[nearest, 2]
